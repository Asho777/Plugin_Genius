...
-- Create user roles table with hierarchy
...
-- Assign permissions to roles (hierarchical inheritance)
WITH role_data AS (
  SELECT id, name, level FROM user_roles
),
permission_data AS (
  SELECT id, name FROM user_permissions
)

-- Admin (Level 1) - All permissions
INSERT INTO role_permissions (role_id, permission_id)
SELECT r.id, p.id
FROM role_data r, permission_data p
WHERE r.name = 'Admin'
